...
"""
Question 6:
The parameter “tpeak” controls when the alpha function peaks after an input
spike occurs (and hence how long the effects of an input spike linger on in the
postsynaptic neuron). “tpeak” for excitatory synapses in the brain may vary from
0.5 ms (AMPA or non-NMDA) to 40 ms (NMDA synapse).

Vary the value of tpeak from 0.5 ms to 10 ms in steps of 0.5 ms and observe how
this influences the output of the neuron for the fixed input spike train used in
this code.

Plot the output spike count as a function of tpeak for the given input spike
train.
"""
nSpikes = []
t_test = []
for t_testpeak in np.linspace(1.0, 10.0, (10.0-1.0)/0.5):
    nS, fig = fire_neuron(spike_train, t_testpeak, t_max) 
    plt.savefig('%s_%2.2f.png' % (sys.argv[1], t_testpeak), dpi=200)
    plt.close(fig)
    nSpikes.append(nS)
    t_test.append(t_testpeak)


fig, axs = plt.subplots(1, 1)
diff_nSpikes = np.diff(nSpikes)
diff_nSpikes = np.insert(diff_nSpikes, 0, 0)
#plt.plot(t_test, diff_nSpikes)
plt.plot(t_test, nSpikes)
plt.plot([0.0, t_test[-1]], [0.0, t_test[-1]], color='r', linestyle='--')
plt.xlabel('tpeak (in ms)')
plt.ylabel('# spikes')
plt.title('nSpikes over t_peak')
plt.savefig('%s_range.png' % sys.argv[1], dpi=200)
plt.close(fig)
